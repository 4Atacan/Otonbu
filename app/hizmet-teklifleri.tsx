...
import { ServiceQuote, SigortaDurum } from '../src/types';
...
export default function HizmetTeklifleriScreen() {
  const { profile } = useSession();
  const { renkler } = useTheme();
  const [talepler, setTalepler] = useState<ServiceQuote[]>([]);
  const [loading, setLoading] = useState(true);
  const [yenileniyor, setYenileniyor] = useState(false);

  const headerOpts = {
    title: 'Hizmet Teklifleri',
    headerShown: true,
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.bosKutu}>
            <Ionicons name="chatbubble-ellipses-outline" size={44} color={renkler.subtext} />
            <Text style={[s.bosBaslik, { color: renkler.text }]}>Teklif talebi yok</Text>
            <Text style={[s.bosAlt, { color: renkler.subtext }]}>
              Müşteriler "teklif usulü" hizmetlerden talep bıraktıkça burada görünür.
            </Text>
          </View>
        }
        renderItem={({ item }) => {
          ...
          return (
            <View style={[s.kart, { backgroundColor: renkler.card }]}>
              <View style={s.kartUst}>
                <Text style={[s.tip, { color: renkler.text }]} numberOfLines={1}>
...
const s = StyleSheet.create({
  ...
  tip: { fontSize: 16, fontWeight: '700', flex: 1 },
